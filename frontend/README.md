...
#### 方式一：使用启动脚本（推荐）
```bash
cd dynamic-gptr/AgentCrew/DeepReader/frontend
...
#### 方式三：手动启动
```bash
# 在 DeepReader 根目录安装依赖
cd dynamic-gptr/AgentCrew/DeepReader
poetry install
...
项目使用 Poetry 进行依赖管理，确保在 DeepReader 根目录存在 `pyproject.toml` 文件：

```
dynamic-gptr/AgentCrew/DeepReader/
├── pyproject.toml          # Poetry 配置文件
├── poetry.lock            # 锁定的依赖版本
├── frontend/              # 前端文件
│   ├── api_server.py
│   ├── start_server.py
│   ├── pyproject.toml.example  # 示例配置文件
│   └── ...
└── backend/               # 后端模块
    └── ...
```

如果您的项目还没有 `pyproject.toml` 文件，可以参考 `frontend/pyproject.toml.example` 创建一个。
...
# 或者
cp frontend/pyproject.toml.example pyproject.toml  # 使用示例配置
poetry install  # 安装依赖
```
...
1. 确保安装了 Poetry 和 Pyenv（可选）
2. 克隆代码并安装依赖：
   ```bash
   cd dynamic-gptr/AgentCrew/DeepReader
   poetry install
   ```
3. 设置环境变量（LLM API 密钥等）
4. 启动开发服务器：
   ```bash
   cd frontend
   poetry run python start_server.py
   ```
5. 访问 http://localhost:8000 进行开发
