...
**Critical Context - User's Core Question (THIS IS YOUR GUIDING STAR):**
{user_core_question}

**Your Primary Directive:**
Your analysis MUST be oriented towards answering the user's core question above. The key idea, conclusion, and evidence you extract should directly serve to illuminate and answer this question.
...
**Your Task:**
Carefully read all the provided material, **keeping the user's core question at the forefront of your analysis**. From this, you MUST extract and articulate the following three key elements:
1.  **Key Idea:** The most important and central core idea or argument that runs through the entire content **and directly relates to the user's question**.
2.  **Key Conclusion:** The primary conclusion the author arrives at, which should be a direct consequence of the Key Idea **and help answer the user's question**.
3.  **Key Evidence:** The most powerful piece of evidence, core argument, or case study the author uses to support the Key Idea and Key Conclusion **that is most relevant to the user's question**.
...
**Critical Context - User's Core Question (THE NORTH STAR FOR YOUR CRITIQUE):**
{user_core_question}

**Your Primary Directive:**
Your critique MUST evaluate whether the current thematic analysis effectively answers the user's core question. If the analysis strays from this question or misses key insights relevant to it, you MUST point this out.
...
**Your Task:**
Based on ALL the provided materials, critically evaluate the current thematic analysis **with a laser focus on the user's core question**:
1. Does the "Key Idea" truly address the user's question? Is it the most central idea **for this specific inquiry**?
2. Is the "Key Conclusion" logically sound and well-supported? Does it help answer the user's question?
3. Is the "Key Evidence" the most impactful one available **for addressing the user's specific question**?

**Output:**
Output Language MUST be in Chinese.
Provide a concise, critical feedback paragraph. Your feedback should be constructive, pointing out specific weaknesses and suggesting concrete areas for improvement or alternative interpretations **especially in relation to the user's core question**. For example: "虽然提出的'核心观点'抓住了书籍的主题，但似乎未能直接回应用户关于X的问题。考虑到用户的问题，更准确的观点应该聚焦于... 另外，'核心证据'虽然有力，但对于回答用户的具体问题，Q&A记录中关于Y的案例更为相关..."
"""
...
**🎯 YOUR NORTH STAR - User's Core Question (EVERY section of your outline MUST serve to answer this):**
{user_core_question}

**Critical Reminder:**
This entire report exists to answer the user's question above. Each section title and content_brief you create MUST be explicitly designed to progressively build toward answering this question. If a section does not directly contribute to answering the user's question, DO NOT include it.

**Provided Material:**
1.  **The Narrative Flow Outline (Thematic Grouping of Chapters):**
    {narrative_outline}

2.  **The Finalized Key Insights (Core Thematic Arguments - already aligned with user's question):**
...
1.  **Question-Driven Structure:** Design the outline as a logical progression toward answering the user's core question. Each top-level section should address a specific aspect of the answer, and each second-level section should provide supporting details.
2.  **Synthesize, Don't Just Combine:** Weave the Narrative Flow and Key Insights together. The outline MUST reflect the book's logic while directly addressing the User's Core Question.
3.  **Structure Constraints:**
    {outline_constraints}
4.  **Be Dense and Precise:** Both `title` and `content_brief` for all levels MUST be as concise and information-dense as possible. Avoid vague or generic phrasing. Each `content_brief` should explicitly connect to the user's question.
5.  **Briefing is Key:** For each title, write a short, one-sentence `content_brief` that clearly states how this section contributes to answering the user's core question.
